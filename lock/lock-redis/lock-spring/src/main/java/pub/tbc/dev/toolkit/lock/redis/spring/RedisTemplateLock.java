...
import pub.tbc.dev.toolkit.lock.redis.RedisLockConstant;
...
public class RedisTemplateLock extends AbstractDistributeLock<RedisTemplateLock> {
    ...
    private RedisScript<Long> RELEASE_SCRIPT_OBJ  = new DefaultRedisScript<>(RedisLockConstant.RELEASE_SCRIPT);
    ...
    public Supplier<Boolean> releaseFunc() {
        return () -> {
            ...
            if (RedisLockConstant.REDIS_RELEASE_OK.equals(result)) {
                return true;
            } else {
    ...
    public Function<Long, Boolean> extendExpireFunc() {
        return expire -> {
            RedisScript<Long> redisScript = new DefaultRedisScript<>(RedisLockConstant.EXTEND_EXPIRE_SCRIPT);
